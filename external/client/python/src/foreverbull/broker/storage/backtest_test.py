...
def test_backtest_list_backtest_results(
    mocked_backtest: Backtest,
):
...
def test_backtest_upload_backtest_result(
    mocked_backtest: Backtest,
):
    mocked_backtest.client.fput_object = Mock()

    mocked_backtest.upload_backtest_result(
        "backtest",
        Mock(),
    )
    mocked_backtest.client.fput_object.assert_called_once_with(
        "backtest-results",
        "backtest",
        "/tmp/result.pkl",
    )
...
def test_backtest_download_backtest_results(
    mocked_backtest: Backtest,
):
    buffer = io.BytesIO()
    df = DataFrame(
    ...
    mocked_backtest.client.get_object.assert_called_once_with(
        "backtest-results",
        "backtest",
    )


def test_backtest_list_backtest_ingestions(
    mocked_backtest: Backtest,
):
    mocked_backtest.client.list_objects = Mock(return_value=[])
...
def test_backtest_upload_backtest_ingestion(
    mocked_backtest: Backtest,
):
    mocked_backtest.client.fput_object = Mock()

    mocked_backtest.upload_backtest_ingestion(
        "local",
        "remote",
    )
    mocked_backtest.client.fput_object.assert_called_once_with(
...
def test_backtest_download_backtest_ingestion(
    mocked_backtest: Backtest,
):
    mocked_backtest.client.fget_object = Mock()

    mocked_backtest.download_backtest_ingestion(
        "remote",
        "local",
    )
    mocked_backtest.client.fget_object.assert_called_once_with(
        "backtest-ingestions",
        "remote",
        "local",
    )
