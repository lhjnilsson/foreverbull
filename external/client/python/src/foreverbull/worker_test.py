...
def setup_worker():
    ...
    def setup(worker: worker.Worker, file_name, execution):
...
def test_worker(workerclass: worker.Worker, setup_worker, parallel_algo, spawn_process):
    ...
    file_name, execution, process_symbols = parallel_algo

    survey_socket, server_socket = setup_worker(workerclass, file_name, execution)
