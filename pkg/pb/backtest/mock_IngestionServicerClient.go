...
func (_m *MockIngestionServicerClient) UpdateIngestion(ctx context.Context, in *UpdateIngestionRequest, opts ...grpc.CallOption) (grpc.ServerStreamingClient[UpdateIngestionResponse], error) {
	_va := make([]interface{}, len(opts))
	for _i := range opts {
		_va[_i] = opts[_i]
	}
	...
	var r0 grpc.ServerStreamingClient[UpdateIngestionResponse]
	var r1 error
	if rf, ok := ret.Get(0).(func(context.Context, *UpdateIngestionRequest, ...grpc.CallOption) (grpc.ServerStreamingClient[UpdateIngestionResponse], error)); ok {
		return rf(ctx, in, opts...)
	}
	if rf, ok := ret.Get(0).(func(context.Context, *UpdateIngestionRequest, ...grpc.CallOption) grpc.ServerStreamingClient[UpdateIngestionResponse]); ok {
		r0 = rf(ctx, in, opts...)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).(grpc.ServerStreamingClient[UpdateIngestionResponse])
		}
	}
