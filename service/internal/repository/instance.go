...
func (db *Instance) Get(ctx context.Context, id string) (*entity.Instance, error) {
	...
	rows, err := db.Conn.Query(ctx,
		`SELECT service_instance.id, image, host, port, broker_port, database_url, functions,
	...
	for rows.Next() {
		...
		err = rows.Scan(&i.ID, &i.Image, &i.Host, &i.Port, &i.BrokerPort, &i.DatabaseURL, &i.Functions,
