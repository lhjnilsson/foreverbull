// Code generated by mockery v2.18.0. DO NOT EDIT.

package http

import (
	gin "github.com/gin-gonic/gin"
	mock "github.com/stretchr/testify/mock"

	pgxpool "github.com/jackc/pgx/v5/pgxpool"
)

// MockRoute is an autogenerated mock type for the Route type
type MockRoute struct {
	mock.Mock
}

// Path provides a mock function with given fields:
func (_m *MockRoute) Path() string {
	ret := _m.Called()

	var r0 string
	if rf, ok := ret.Get(0).(func() string); ok {
		r0 = rf()
	} else {
		r0 = ret.Get(0).(string)
	}

	return r0
}

// Setup provides a mock function with given fields: group, pool
func (_m *MockRoute) Setup(group *gin.RouterGroup, pool *pgxpool.Pool) error {
	ret := _m.Called(group, pool)

	var r0 error
	if rf, ok := ret.Get(0).(func(*gin.RouterGroup, *pgxpool.Pool) error); ok {
		r0 = rf(group, pool)
	} else {
		r0 = ret.Error(0)
	}

	return r0
}

type mockConstructorTestingTNewMockRoute interface {
	mock.TestingT
	Cleanup(func())
}

// NewMockRoute creates a new instance of MockRoute. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewMockRoute(t mockConstructorTestingTNewMockRoute) *MockRoute {
	mock := &MockRoute{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
