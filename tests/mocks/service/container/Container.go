...
// Start provides a mock function with given fields: ctx, _a1, serviceName, image, containerID
func (_m *Container) Start(ctx context.Context, _a1 *config.Config, serviceName string, image string, containerID string) (string, error) {
	ret := _m.Called(ctx, _a1, serviceName, image, containerID)

	var r0 string
	if rf, ok := ret.Get(0).(func(context.Context, *config.Config, string, string, string) string); ok {
		r0 = rf(ctx, _a1, serviceName, image, containerID)
	} else {
		r0 = ret.Get(0).(string)
	}

	var r1 error
	if rf, ok := ret.Get(1).(func(context.Context, *config.Config, string, string, string) error); ok {
		r1 = rf(ctx, _a1, serviceName, image, containerID)
	} else {
		r1 = ret.Error(1)
	}
